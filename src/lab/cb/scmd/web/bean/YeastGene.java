...
/**
 * mutantのORF, standardname, annoationの情報をもつクラス
 * @author leo
 * 
 */
public class YeastGene
{
    ...
    {
        Pattern p = Pattern.compile("(y[a-z]{2}[0-9]{3})([wc])(-[a-z])?",
                Pattern.CASE_INSENSITIVE);
        Matcher m = p.matcher(orf);
        String orfString;
        if (m.matches())
        {
            orfString = m.group(1).toUpperCase() + m.group(2).toLowerCase()
    ...
    public String getAnnotation()
    {
        ...
        {
            while (annotation.length() > endplace
                    && annotation.charAt(endplace) != ' ')
                endplace++;
