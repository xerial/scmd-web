...
public class DBManager implements SCMDManagerModel
{
    ...
    public boolean initialize()
    {
        ...
        try
        {
            System.out.println(SCMDConfiguration.getProperty("SCMD_ROOT")
                    + SCMDConfiguration.getProperty("DB_SQLCONFIGPATH"));
            sqlManager = new SqlManager(SCMDConfiguration.getProperty("SCMD_ROOT")
                    + SCMDConfiguration.getProperty("DB_SQLCONFIGPATH").trim(), "");
        }
        catch (Exception e)
        {
            return false;
    ...
    /**
     * 
     * SQLManagerを使用してSQLを発行する 戻り値が必要ないINSERTやUPDATEの場合に使用する
     * 
     * @param name
     * @param objs
     */
    ...
    /**
     * StatementのexecuteQueryと同じ動作をする 直接SQL文章を書きたい場合に使う
     * 
     * @param sql
     * @return
     * @throws SQLException
     ...
     *            置換する変数
     * @param cast
     *            クラスを指定されるとそこのSetterにデータを入れてListで返す
     * @return １つのクラス
     * @throws SQLException
     */
    ...
    /**
     * 指定したカラム名のオブジェクトが返る DBでINTEGER型だと JAVAではINTEGER型 DBでTEXT型だと
     * JAVAではString型となる
     * 
     * @param name
     * @param map
     ...
     * @throws SQLException
     */
    public BasicTable queryBasicTable(String name, HashMap<String, String> map, String keyColumnName)
            throws SQLException
    {
        Connection con = pooling.getPoolConnection();
