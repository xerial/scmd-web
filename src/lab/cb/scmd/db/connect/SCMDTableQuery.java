...
public class SCMDTableQuery implements TableQuery
{
    ...
    public Table getShapeDataSheet(String strain, int photoID, int cellID, int sheetType)
    {
        ...
        try
        {
            table = _connection.getDataSheet(strain, photoID, cellID, DataSheetType.getDetailParameters(sheetType));
        }
     ...
     */
    public Table getGroupByDatasheet(String orf, int sheetType, int stainType, String groupName, int page,
            int numElementInAPage)
    {
        ...
        String sql = "SELECT " + selectParam + " FROM " + SCMDConfiguration.getProperty("DB_INDIVIDUAL")
                + " WHERE strainname='" + orf.toUpperCase() + "' AND \"" + StainType.getStainTypeName(stainType)
                + "group\"='" + groupName + "' ORDER BY cell_local_id";
    ...
    protected Table evalSQL(String sql)
    {
        ...
        try
        {
            result = DBManager.convertTable(SCMDManager.getDBManager().queryExecute(sql));
        }
        catch (SQLException e)
        {
            e.printStackTrace();
    ...
    public Table getShapeZScoreTable(ParamPair[] paramSets)
    {
        ...
        String sql_param = "SELECT id, displayname FROM " + SCMDConfiguration.getProperty("DB_PARAMETERLIST")
                + " WHERE ";
        ...
        for (int i = 0; i < paramSets.length; i++)
        {
            ...
            /* */
            sql += "," + quote(paramname);
            String subsql = "SELECT strainname, paramid, groupid, zscore AS " + quote(key) + " FROM "
    ...
    public Table getGroupAvgSDTable(ParamPair[] paramSets)
    {
        ...
        for (int i = 0; i < paramSets.length; i++)
        {
            ...
            if (i != 0)
                parampair += " OR ";
            parampair += "( paramid=" + pair.getParamid() + "AND groupid=" + pair.getGroupid() + ")";
        }
        map.put("parampair", parampair);
        return evalSQL("lab.cb.scmd.db.connect.SCMDTableQuery:getGroupAvgSDTable", map);
        /*
    ...
    public Table getAnalysisAVGandSD()
    {
        ...
        return evalSQL("lab.cb.scmd.db.connect.SCMDTableQuery:getAnalysisAVGandSD", null);
    }
    ...
    public Table getSelectedAnalysisValue(String[] orf)
    {
        ...
        map.put("orfs", sql);
        return evalSQL("lab.cb.scmd.db.connect.SCMDTableQuery:getSelectedAnalysisValue", map);
    }
    ...
    public List<MorphParameter> getParameterList(String scope, String datatype) throws SQLException
    {
        ...
        List<MorphParameter> result = SCMDManager.getDBManager().queryResults("TableQuery:getParameterList", map,
    ...
    public List<MorphParameter> getParameterInfo(Set<Integer> parameter)
    {
        ...
        try
        {
            result = SCMDManager.getDBManager().queryResults("SCMDTableQuery.getParameterInfo", map,
                    MorphParameter.class);
    ...
    public MorphParameter getOneParameterInfo(String param, String scope)
    {
        ...
        try
        {
            result = SCMDManager.getDBManager().queryResults("TableQuery:getOneParameterInfo", map,
                    MorphParameter.class);
            // result = (List<MorphParameter>) ConnectionServer.query(new
            // BeanListHandler(MorphParameter.class), sql);
        }
